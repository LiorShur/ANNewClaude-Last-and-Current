// FIXED: GPS tracking with consolidated trail guide generation
...
Would you like to save this route?`;
  ...
  // FIXED: Consolidated save route with proper cloud integration
  async saveRoute() {
    try {
  ...
  // Ask user about cloud save options
...
Choose an option:`;
  ...
  // FIXED: Consolidated cloud save method
  async saveRouteToCloud(routeData, routeInfo, accessibilityData, authController) {
    try {
      ...
      this.showSuccessMessage(`✅ "${routeInfo.name}" saved to cloud as ${visibilityText} trail guide! ☁️`);
  ...
  // FIXED: Consolidated trail guide generation (single method, no duplicates)
  async generateTrailGuide(routeId, routeData, routeInfo, accessibilityData, authController) {
    try {
      ...
      // Determine if trail guide should be public
      const isPublic = routeInfo.makePublic || false;
      ...
      const trailGuideDoc = {
        ...
        // Add publication timestamp only if made public
        ...(isPublic && {
        ...
        // Community features (initialized with zeros)
        community: {
          views: 0,
          downloads: 0,
          ratings: [],
      ...
      console.log(`✅ ${visibilityText.toUpperCase()} trail guide generated with ID: ${guideRef.id}`);
      
      return guideRef.id;
      
    } catch (error) {
      console.error('❌ Failed to generate trail guide:', error);
      // Don't throw - let the route save succeed even if guide generation fails
      return null;
    }
  }
