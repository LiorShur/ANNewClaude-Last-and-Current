// FIXED: Storage controller with improved timer sync and map restoration
...
export class AppState {
  ...
  // FIXED: Improved timer synchronization in autoSave
  async autoSave() {
    let currentElapsed = this.elapsedTime;
    
    // CRITICAL FIX: Always prioritize live timer value when tracking
    if (this.isTracking) {
      const app = window.AccessNatureApp;
      const timer = app?.getController('timer');
      if (timer) {
        // Get the live elapsed time from timer
        currentElapsed = timer.getCurrentElapsed();
        // Keep internal state synchronized
        this.elapsedTime = currentElapsed;
        console.log(`🔄 Timer sync: ${Math.floor(currentElapsed/1000)}s`);
      } else {
        console.warn('⚠️ Timer controller not available, using stored elapsed time');
      }
    }
  ...
  restoreFromBackup(backupData) {
    ...
    try {
      ...
      // FIXED: Restore route on map with progressive retry
  ...
  // FIXED: Progressive retry with exponential backoff for map restoration
  redrawRouteOnMap() {
    const attemptRedraw = (attempt = 1, maxAttempts = 5) => {
      try {
        console.log(`🗺️ Map redraw attempt ${attempt}/${maxAttempts} - ${this.routeData.length} points`);
        
        const app = window.AccessNatureApp;
        const mapController = app?.getController('map');
        
        if (mapController && this.routeData.length > 0) {
          console.log('🗺️ Map controller ready, calling showRouteData...');
          mapController.showRouteData(this.routeData);
          console.log(`✅ Map redraw successful on attempt ${attempt}`);
          return;
        }
        
        // If map not ready and we have retries left
        if (attempt < maxAttempts) {
          const delay = attempt * 500; // Progressive backoff: 500ms, 1s, 1.5s, 2s, 2.5s
          console.log(`⏳ Map not ready, retrying in ${delay}ms (attempt ${attempt + 1}/${maxAttempts})`);
          
          setTimeout(() => attemptRedraw(attempt + 1, maxAttempts), delay);
        } else {
          console.error('❌ Failed to redraw route after maximum attempts');
          // Show user-friendly error
          const showError = confirm(
            '⚠️ Route data loaded but map display failed.\n\n' +
            'Click OK to refresh the page and try again, or Cancel to continue without map display.'
          );
          if (showError) {
        ...
        console.error(`❌ Map redraw error on attempt ${attempt}:`, error);
        
        if (attempt < maxAttempts) {
          const delay = attempt * 500;
          setTimeout(() => attemptRedraw(attempt + 1, maxAttempts), delay);
        } else {
          console.error('❌ Map redraw failed completely:', error);
        }
      }
    };
    
    // Start the progressive retry
    attemptRedraw();
  }
